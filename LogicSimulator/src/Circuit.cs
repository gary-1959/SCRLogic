...
namespace SharpCircuit {
	...
	public class Circuit {
        ...
        // CONTRELEC Added Events
        public event EventHandler AnalysisStarted;
        public delegate void AnalysisStartedEventHandler(EventArgs e);
        public void OnAnalysisStarted(EventArgs e)
        {
            EventHandler handler = AnalysisStarted;
            if (handler != null)
            {
                handler(this, e);
            }
        ...
        public delegate void AnalysisCompleteEventHandler(EventArgs e);
        public void OnAnalysisComplete(EventArgs e)
        {
            EventHandler handler = AnalysisComplete;
            if (handler != null)
            {
                handler(this, e);
            }
		...
		public double[][] circuitMatrix; // contains circuit state
		...
		public void doTick(string name="Unknown") {
            try
            {
                doTicks(1, name);
            }
            ...
            {
                throw new Exception(ex.Message, ex.element);
            }
			
		}

		public void doTicks(int ticks, string name) {
		...
		void tick(string name) {
			...
			for(subiter = 0; subiter != subiterCount; subiter++) {
				...
				for(int j = 0; j != circuitMatrixSize; j++) {
					for(int i = 0; i != circuitMatrixSize; i++) {
                        ...
                        if (Double.IsNaN(x) || Double.IsInfinity(x))
                        {
                            ...
                            throw new Exception("NaN/Infinite matrix!", null);
                        }
                    }
				}
				...
				if(circuitNonLinear) {
                    ...
                    if (!lu_factor(circuitMatrix, circuitMatrixSize, circuitPermute))
                    {
                        ...
                        //throw new Exception("Singular matrix!", null);
                    }
                }
			...
			if(subiter == subiterCount)
            {
                ...
                Debug.LogF("Circuit failed to converge after {0} iterations ({1}).", subiter, name);
                throw new Exception("Convergence failed!", null);
            }
            else
            {
                if (subiter > 1) Debug.LogF("Circuit converged after {0} iterations ({1}).", subiter, name);
            }
				

			time = Math.Round(time + timeStep, 12); // Round to 12 digits
		}
        //CONTRELEC added analyze wrapper

        public void analyze()
        {
            OnAnalysisStarted(new EventArgs());
            try
            {
                w_analyze();
            }
            ...
            OnAnalysisComplete(new EventArgs());
        }

		public void w_analyze() {
			...
			for(int i = 0; i != elements.Count; i++) 
			{
				...
				if(ce is CurrentSource) {
					...
					if(!fpi.findPath(ce.getLeadNode(0)))
                    {
                        ...
                        throw new Exception("No path for current source!", null);
                    }
				...
				if((ce is Voltage && ce.getLeadCount() == 2) || ce is Wire) {
                    ...
                    if (fpi.findPath(ce.getLeadNode(0)))
                    {
                        ...
                        throw new Exception("Voltage source/wire loop with no resistance!", null);
                    }
				...
				if(ce is CapacitorElm) {
					...
					if(fpi.findPath(ce.getLeadNode(0))) {
                        ...
                        if (fpi.findPath(ce.getLeadNode(0)))
                        {
                            ...
                            throw new Exception("Capacitor loop with no resistance!", null);
                        }
			...
			for(int i = 0; i != matrixSize; i++) {
				...
				if(leadX == matrixSize) {
					...
					if(qp == -1)
                    {
                        ...
                        throw new Exception("Matrix error", null);
                    }
